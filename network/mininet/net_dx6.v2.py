...
def run():
    ...
    net['h1'].cmd("ip -6 rule add from fc00:2::2 table 1")
    net['h1'].cmd("ip -6 route add fc00:2::0/64 dev h1-eth2 scope link table 1")
    net['h1'].cmd("ip -6 route add default via fc00:2::1 dev h1-eth2 table 1")

    net['h1'].cmd("ip -6 rule add from fc00:4::2 table 2")
    net['h1'].cmd("ip -6 route add fc00:4::0/64 dev h1-eth3 scope link table 2")
    net['h1'].cmd("ip -6 route add default via fc00:4::1 dev h1-eth3 table 2")

    net['h1'].cmd("ip -6 route add default scope global nexthop via fc00:2::1 dev h1-eth2")
